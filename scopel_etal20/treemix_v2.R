## 2 January 2021
## The following script is an adaptation from treemix.R script created by Andrew Parker Morgan. The original version is available at https://rdrr.io/github/andrewparkermorgan/popcorn/src/R/treemix.R
##
## --- treemix.R ---
## 1 March 2015
...
plot.treemix <- function(stem, o = NA, flip = vector(), draw = TRUE, branch.colour = "grey", 
                         plot.nodes = TRUE, plot.migration = TRUE, legend.mig = TRUE, clade_color, aneup_clade_color, subset.mig = NA, plot.title = NA,...) {
  ...
  p <- ggplot(stuff$tips) +
    ...
    geom_label2(aes(subset = 
                      ...
                      pop == "French_dairy" |
                      pop == "Mixed_origin" |
                      pop == "Sake",x = xo, y = y, label = pop), hjust = 0, fill = "black", alpha = 0.1, label.size = 0,size=5, colour = aneup_clade_color) +
    geom_text2(aes(subset = 
                      ...
                      pop != "French_dairy" &
                      pop != "Mixed_origin" &
                      pop != "Sake", x = xo, y = y, label = pop), hjust = 0,size=5, colour = clade_color) +
    theme_classic()+
  ...
  if (plot.migration){
    if(!is.na(subset.mig)){
      p <- p + geom_segment(data = subset(stuff$edges[subset.mig,], type == "MIG"),
    ...
    if (legend.mig) {
    p <- p + theme(legend.position = c(0.1,0.8), legend.background = element_blank())
    }
    else{
    p <- p + geom_segment2(data = subset(stuff$edges, type == "MIG"),
                          aes(x = from.x, y = from.y, xend = to.x, yend = to.y, colour = weight),
                          arrow = arrow(length = unit(6, "points")),
                          alpha = 0.7) +
      scale_colour_gradient2("migration weight", high = "red", mid = "yellow")+
  ...
  print( list(vertices = d, edges = e, plot = p, stuff = stuff) )  
  poutput <- return(p)
